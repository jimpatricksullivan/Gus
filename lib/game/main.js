...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		//TODO better system for choosing level
	...
	update: function() {		
		...
		if( this.player && !this.scrolled) {
			...
			this.screen.y = Math.min(
					...
					(this.level.layer[2].height*16 - ig.system.height) // the bottom // TODO this.map.data is an easier way to reference map
			);
		}
	...
	drop: function(droppableType) {
		...
		//TODO call a function in player to tell you if it's ok to drop and where to drop
	...
	destroyBlock: function(x, y) {
		if (this.coordinatesInBounds(x,y)) {
			...
			this.level.layer[2].data[y][x] = 0; // TODO this.map.setTile()
			this.level.layer[1].data[y][x] = 0; // TODO this.collisionMap.setTile() 
	...
	// add gravel
	createAntigrav: function(y, x) {
		// TODO add antigravity block
	},
	...
	coordinatesInBounds: function(x, y) {
		if (x >= 0 && x <= 5 && y >= 0 && y < this.level.layer[2].height) {
			return true;
		}
		return false;
	},
	...
	loadMine: function() {
		...
		// prep tutorial TODO make separate tutorial class!
		if (this.tutorial) {
			this.tutorialText = [
