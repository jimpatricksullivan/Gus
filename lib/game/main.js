...
MyGame = ig.Game.extend({
	...
	update: function() {		
		...
		if( this.player && !this.scrolled) {
			...
			this.screen.y = Math.min(
					...
					(this.map.height*16 - ig.system.height) // the bottom // TODO this.map.data is an easier way to reference map
			);
		}
	...
	loadLevel: function( ) {
		...
		var cMap = [];
		var bgMap = []
		for (row in this.testLevel.map) {
		    cMap[row] = [];
		    bgMap[row] = [];
		    for (column in this.testLevel.map[row]) {
				if (this.testLevel.map[row][column] == 0) {
				    cMap[row][column] = 0;
				} else {
				    cMap[row][column] = 1;
		...
		// tile map	
		var newMap = new ig.BackgroundMap(16, this.testLevel.map, 'media/minetiles2.png');
		newMap.anims = this.backgroundAnims['media/minetiles2.png'] || {};
		newMap.repeat = false;
		newMap.distance = 1;
		newMap.foreground = false;
