...
EntityDroppable = EntityTiled.extend({
	...
	init: function( x, y, settings ) {
		...
		this.sound = new ig.Sound( 'media/sounds/'+this.type.split("-")[0]+'.*' ); 
		this.animSheet = new ig.AnimationSheet( 'media/droppable-'+ this.type +'.png', 8, 8 ), 
		this.addAnim( 'idle', 1, [0] );
		if (this.type == "gravel" || this.type == "anti-grav") {
			this.bounciness = 0.0;
		}
	...
	update: function() {
		...
		if (this.standing) {
			if (this.type == "gravel") {
				this.sound.play();
				ig.game.createGravelBlock(this.tile.x, this.tile.y);
				this.receiveDamage(10);
			} else if (this.type == "anti-grav") {
				this.sound.play();
				//TODO animate block hovering into place
				ig.game.createAntigravBlock(this.tile.x, this.tile.y);
